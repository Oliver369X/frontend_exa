...
export async function POST(request: Request) {
  try {
    const { message, previousMessages, designData } = await request.json();
    ...
    const chatHistory = previousMessages?.map((msg: {role: string, content: string}) => ({
    ...
    let prompt = `
Eres un experto en desarrollo Angular. Tu tarea es generar código Angular de alta calidad basado en la descripción del usuario.
...
`;
    ...
    // Agregar datos del diseño si existen
    if (designData) {
      prompt += `
Diseño visual del proyecto:
- HTML: ${designData.html?.substring(0, 1000) || 'No disponible'}
- CSS: ${designData.css?.substring(0, 1000) || 'No disponible'}
- Componentes: ${JSON.stringify(designData.components?.slice(0, 3) || [])}
